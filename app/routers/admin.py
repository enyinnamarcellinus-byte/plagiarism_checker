...
def activate_user(user_id: int, db: Session = Depends(get_db), admin: User = Depends(admin_only)):
    target = db.get(User, user_id)
    if not target:
        raise HTTPException(status_code=404, detail="User not found")
...
def change_role(user_id: int, role: Role, db: Session = Depends(get_db), admin: User = Depends(admin_only)):
    target = db.get(User, user_id)
    if not target:
        raise HTTPException(status_code=404, detail="User not found")
    target.role = role
...
def get_audit_logs(limit: int = 100, db: Session = Depends(get_db), admin: User = Depends(admin_only)):
...
def admin_dashboard(request: Request, db: Session = Depends(get_db), user: User = Depends(admin_only)):
