...
def _score_structural(similarity_score: float, overlap_ratio: float, order_preserved: float) -> float:
...
def _order_preserved(fragments: list[Fragment]) -> float:
    ...
    inversions = sum(
        1 for i in range(len(positions_b))
...
def _result(scores: dict) -> ClassificationResult:
    ...
    return ClassificationResult(predicted_type=predicted, **{f"score_{k}": v for k, v in scores.items()})